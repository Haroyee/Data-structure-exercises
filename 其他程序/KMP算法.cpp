...
int main()
{
	...
	if(int n =Index_KMP(S,T,1)) printf("KMP搜索完成，共匹配%d次,共匹配%d趟,位置为%d-%d\n",KMPcount1,KMPcount2+1,n,n+T.length);
	else printf("KMP无搜索结果！\n");
	
	if(int m =Index_BF(S,T,1)) printf("BF搜索完成，共匹配%d次,共匹配%d趟,位置为%d-%d\n",BFcount1,BFcount2+1,m,m+T.length);
	else printf("BF无搜索结果！\n");
